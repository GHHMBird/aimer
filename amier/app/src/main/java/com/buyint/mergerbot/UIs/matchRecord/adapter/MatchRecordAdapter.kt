...
import com.buyint.mergerbot.UIs.matchRecord.activity.MatchRecordMatchProjectForYouActivity
 ...
 */
class MatchRecordAdapter(var context: Context, var all: Boolean, var list: ArrayList<MatchRecordListModel>?) : RecyclerView.Adapter<MatchRecordAdapter.ViewHolder>() {
    ...
    private fun setViewType0Data(list: ArrayList<MatchRecordListModel>?, holder: ViewHolder, position: Int) {
        ...
        if (response!!.model.matchRecordPersonId == model.personId) {

            holder.more!!.visibility = View.INVISIBLE

            holder.line2!!.visibility = View.GONE
            holder.clientll!!.visibility = View.GONE
            holder.friendNum!!.text = model.suggest.toString()
            holder.projectNum!!.text = model.project.toString()
            holder.text1!!.text = context.getString(R.string.project_with_you)
            holder.text2!!.text = context.getString(R.string.referral_request)

            holder.clickView!!.setOnClickListener { }
            holder.clientll!!.setOnClickListener { }
            holder.friendll!!.setOnClickListener { }
            holder.projectll!!.setOnClickListener {
                val intent = Intent(context, MatchRecordMatchProjectForYouActivity::class.java)
                intent.putExtra(context.getString(R.string.DATA), model.personId)
                context.startActivity(intent)
            }

            if (BaseActivity.themeColor == 3) {
                holder.colorView!!.setBackgroundResource(R.drawable.blue_left)
            } else {
